...
func TestConvertToNovaRequestPreservesOutputLimitPresence(t *testing.T) {
	explicitZero := uint(0)
	legacyLimit := uint(32)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			converted := convertToNovaRequest(&test.request)
			encoded, err := common.Marshal(converted)
			require.NoError(t, err)

			field := gjson.GetBytes(encoded, "inferenceConfig.maxTokens")

...
func TestRequestOpenAI2XunfeiPreservesOutputLimitPresence(t *testing.T) {
	explicitZero := uint(0)
	legacyLimit := uint(24)
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			converted := requestOpenAI2Xunfei(test.request, "app", "domain")
			encoded, err := common.Marshal(converted)
			require.NoError(t, err)

			field := gjson.GetBytes(encoded, "parameter.chat.max_tokens")

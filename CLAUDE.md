...
- For repeated local starts, reuse the same SQLite database unless the user explicitly asks for a fresh database or a different data state. The default local SQLite path is `SQLITE_PATH=./data/local/new-api-local.db?_busy_timeout=30000`; create `data/local` before starting if it does not exist.

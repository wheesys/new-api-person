...
function saveAndOpenErrorLog() {
  try {
    ...
    const logFileName = `new-api-person-crash-${timestamp}.log`;
...
function getBinaryPath() {
  ...
  if (isDev) {
    const binaryName = platform === 'win32' ? 'new-api-person.exe' : 'new-api-person';
  ...
  switch (platform) {
    case 'win32':
      binaryName = 'new-api-person.exe';
      break;
    case 'darwin':
      binaryName = 'new-api-person';
      break;
    case 'linux':
      binaryName = 'new-api-person';
      break;
...
function startServer() {
  return new Promise((resolve, reject) => {
    ...
    env.SQLITE_PATH = path.join(dataDir, 'new-api-person.db');

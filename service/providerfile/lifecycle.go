...
func Upload(ctx context.Context, request UploadRequest) (File, error) {
	...
	providerLookupHMAC, encryptedPayload, payloadKeyVersion, err := providerReferenceEvidence(ctx, request, activeBinding, handle, uploadMetadata)
	if err != nil {
		return File{}, ErrLifecycleUnavailable
	}
...
func providerReferenceEvidence(ctx context.Context, request UploadRequest, binding contextconsensus.ManagedProviderFileUploadBinding, handle string, metadata openai.ProviderFileMetadata) (string, []byte, string, error) {
...
func recordVerificationFailure(ctx context.Context, request UploadRequest, lifecycle *model.ManagedProviderFileLifecycle, binding contextconsensus.ManagedProviderFileUploadBinding, handle string, metadata openai.ProviderFileMetadata, reason string) (File, error) {
	providerLookupHMAC, encryptedPayload, _, err := providerReferenceEvidence(ctx, request, binding, handle, metadata)
	if err != nil {
		return File{}, ErrLifecycleUnavailable
	}

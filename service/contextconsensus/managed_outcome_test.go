...
func TestConfirmManagedOutcomeCommitMatchesOnlyExactNextState(t *testing.T) {
	...
	committed, currentAtExpected, err := runtime.ConfirmManagedOutcomeCommit(context.Background(), request.Owner, request.ExternalContextID, request.ExpectedRevision, state)
	require.NoError(t, err)
	assert.True(t, committed)
	assert.False(t, currentAtExpected)
	changed := state
	changed.SourceDigest = "different"
	committed, _, err = runtime.ConfirmManagedOutcomeCommit(context.Background(), request.Owner, request.ExternalContextID, request.ExpectedRevision, changed)
	require.NoError(t, err)

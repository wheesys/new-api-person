...
func TestUpdateVideoTasksSlowChannelDoesNotBlockOtherChannels(t *testing.T) {
	...
	fastFirstTaskID := fastFirst.GetUpstreamTaskID()
	fastSecondTaskID := fastSecond.GetUpstreamTaskID()
	...
	gopool.Go(func() {
		errCh <- UpdateVideoTasks(context.Background(), constant.TaskPlatform("kling"), map[int][]string{
			...
			slowTaskID:       slowTask,
			fastFirstTaskID:  fastFirst,
			fastSecondTaskID: fastSecond,
		})
	})
	...
	require.Eventually(t, func() bool {
		...
		return len(fetchedTaskIDs) == 2 &&
			fetchedTaskIDs[0] == fastFirstTaskID &&
			fetchedTaskIDs[1] == fastSecondTaskID

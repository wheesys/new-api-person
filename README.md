...
Managed OpenAI provider files are disabled by default and require an explicitly configured, gateway-exclusive official OpenAI channel with exactly one API key. Upload with `POST /v1/files` using `purpose=user_data`, one `file` part, and a unique `X-New-Api-File-Idempotency-Key`; retrieve metadata with `GET /v1/files/{opaque-handle}`. Native `POST /v1/responses` requests may use that opaque handle in typed `input_file.file_id` content. Raw provider file IDs, custom upstream origins, protocol conversion, load balancing, retries, and failover are rejected. Expiry cleanup uses a bounded background deletion outbox with strict provider receipts and does not expose a client DELETE API. File listing, content retrieval, client deletion, and production enablement remain unavailable until the real sandbox, exclusive Project, reconciliation, and immutable-audit readiness gates are completed.

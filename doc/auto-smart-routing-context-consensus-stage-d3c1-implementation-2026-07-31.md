...
本阶段未调用真实 OpenAI、未执行生产配置或数据库数据修改，也未开放客户端 DELETE 接口。`provider_file_sandbox_contract_verified` 默认且必须为 `false`；真实 sandbox、独占 Project、孤儿对账、三库实库矩阵和部署侧不可变审计完成前，上传和后台删除均不能通过生产 readiness。
...
- 成功终态清除加密 provider reference；未知或失败终态保留加密引用，供 D-3c.2 的人工核验和有界对账使用。
...
- 应用内 HMAC 事件链提供篡改检测，但不等价于不可篡改审计；生产启用仍要求部署侧权限隔离或 WORM 审计证据。
- `provider_file_sandbox_contract_verified` 是新增硬门禁，默认关闭；它不代表配置者可自行跳过 D-3c.2 的真实验收。
...
- D-3c.2：在真实隔离 sandbox 验证上传、读取、首次与重复删除、404、429/5xx、超时、到期和 Project 隔离合同。
- 接入并冻结网关独占 OpenAI Project 身份，完成有界 orphan reconciliation、MySQL/PostgreSQL 实库矩阵、部署侧不可变审计和生产 readiness 证据。
- 任何真实 sandbox 或生产状态修改仍需按环境安全规则单独确认后执行。

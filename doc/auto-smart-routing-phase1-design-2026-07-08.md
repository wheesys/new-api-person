...
渠道评分中，渠道倍率通过 `EstimatedQuota` 影响 `cost_score`；渠道表中的一次性测试 `ResponseTime` 不再直接参与智能调度，只有至少 3 次有效真实请求延迟产生的按“渠道 + 模型”延迟 EWMA 才影响 `latency_score`，样本不足时使用中性分。每次实际上游尝试独立计时：流式请求使用首次上游响应数据的 TTFT，非流式请求和异步任务提交使用单次尝试完整耗时；流式成功但没有有效首包时不写入延迟样本。原始 HTTP 首包只标记当前尝试，请求级首包仍需有效流事件确认，避免失败重试的错误正文污染最终 TTFT。渠道 `Weight` 不再作为吞吐量，尚无真实吞吐数据时 `throughput_score` 固定为中性分，不制造候选差异。`reliability_score` 来自同维度的运行时成功 EWMA，并结合多 Key 渠道可用 Key 比例。任务匹配、上下文余量、缓存亲和、重置窗口和会话亲和分别保留独立审计因子。

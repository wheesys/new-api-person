...
- 渠道：实际可调用的上游接入配置，包含上游密钥、基础地址、渠道类型、可用模型、可用分组、优先级、权重、状态、多 Key、参数覆盖等。一个供应商可以有多个渠道。
- 供应商：模型资产目录中的元数据分类，用于展示、过滤、统计和价格页归属。供应商本身不保存上游密钥，也不直接参与请求转发。
- 模型：用户请求和渠道能力匹配的核心字符串，同时也有独立的模型元数据表用于描述、图标、标签、端点和供应商归属。
...
Pricing 聚合 Model + Vendor + Ability + 倍率配置
前端展示模型目录、定价、供应商筛选、端点支持
```
...
| Vendor | 管理员配置 | Model、Pricing | 供应商展示元数据 |
...
- 为“渠道、供应商、模型、能力”的概念关系补一张正式架构图，减少配置误解。

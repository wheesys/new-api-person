...
- [ ] 复测 VChart alias 修复后的数据看板页面，确认 `/console` 不再出现 `createCanvas` / `filter` 控制台错误和页面渲染错误。
- [ ] 重新执行管理员登录后的后台点击验证：`/console`、`/console/channel`、`/console/models`、`/pricing`。
- [ ] 复测渠道新增弹窗的渠道倍率字段；如本地存在渠道数据，再补测渠道编辑弹窗的倍率字段。
- [ ] 复测模型新增弹窗无供应商字段，模型管理页无 `/api/vendors` 请求。
- [ ] 完成最终验证后提交代码，并按仓库规则在 `git commit` 后立即执行 `git push`。

...
- HMAC 绑定精确 target fingerprint、scope fingerprint、credential fingerprint、密钥版本、声明时间和硬过期时间。
...
- 关闭新上传不会停止删除 worker 或只读恢复对账；维护路径仍要求同一份有效的 target/scope/credential 签名 readiness。
...
矩阵验证覆盖 AutoMigrate、Project 与 target lookup 列、credential-bound readiness 写入和精确查询、整数主键、唯一索引、完整扫描事务、候选写入和 stale CAS 拒绝。两个项目专用临时容器已在测试后删除，仅下载的版本镜像仍保留在本机缓存。
...
1. 在真实隔离 OpenAI Project 验证正确、缺失和错误 Project，跨 Project 可见性，上传后 LIST/GET 延迟与分页稳定性，首次及重复 DELETE、404、429/5xx、超时、删除后读取和真实到期。

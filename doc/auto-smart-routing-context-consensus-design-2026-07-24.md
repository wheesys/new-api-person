...
下一步完成阶段 D-3c.2：在真实隔离 sandbox 验证 OpenAI Files 上传、读取、删除、到期和 Project 隔离合同，完成网关独占 Project、孤儿对账、三库实库矩阵、不可变审计和生产 readiness。诊断仍不得展示原始 call ID、函数名、参数、结果、schema、digest、文件引用、摘要正文或任何凭据；生产静态脱敏策略仍需针对服务端实际拥有的精确工具/schema 单独评审后通过代码登记。

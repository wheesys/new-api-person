...
实施状态：阶段 D-1 和 D-2 已完成工具调用消息压缩与后台统计。阶段 D-3 已完成 OpenAI 文件上传、查询、Responses 文件引用和到期自动删除。原先增加的独占 Project、沙盒证明、外部存储证明、短期签名证明和未知文件扫描已经删除，见 `doc/openai-provider-file-simplification-2026-08-01.md`。文件功能仍默认关闭，管理员完成正常配置后即可启用。
...
阶段 D 已完成。后续新增功能时继续保证日志不记录凭据、完整工具结果、文件原始标识或摘要正文。

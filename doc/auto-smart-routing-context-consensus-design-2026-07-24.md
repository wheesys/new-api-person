...
下一步完成阶段 D-3：仅在 adaptor 提供权威所有权、到期和删除能力后实现 provider file 生命周期。诊断仍不得展示原始 call ID、函数名、参数、结果、schema、digest、文件引用、摘要正文或任何凭据；生产静态脱敏策略仍需针对服务端实际拥有的精确工具/schema 单独评审后通过代码登记。

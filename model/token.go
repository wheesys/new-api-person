...
type Token struct {
	...
	AllowContextCompaction bool           `json:"allow_context_compaction" gorm:"comment:Whether this API token permits request-local context compaction"`
...
func (token *Token) Update() (err error) {
	err = DB.Model(token).Select("name", "status", "expired_time", "remain_quota", "unlimited_quota",
		"model_limits_enabled", "model_limits", "allow_ips", "group", "cross_group_retry", "allow_context_compaction", "auto_groups").Updates(token).Error
	if shouldUpdateRedis(true, err) {

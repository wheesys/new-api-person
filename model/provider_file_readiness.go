...
const (
	...
	ManagedProviderFilePolicyVersion             = "provider-file-policy-v1"
)
...
type ManagedProviderFileReadinessEvidence struct {
	...
	MaintenancePolicyFingerprint string    `json:"-" gorm:"type:varchar(64);not null;default:'';index:idx_provider_file_readiness_lookup,priority:4;comment:HMAC binding deletion batch and timeout policy"`
	UploadPolicyFingerprint      string    `json:"-" gorm:"type:varchar(64);not null;default:'';comment:HMAC binding upload expiry metadata verification deletion lead and retry policy"`
	...
	PolicyVersion                string    `json:"policy_version" gorm:"type:varchar(64);not null;default:'';comment:Version of the provider file lifecycle policy binding contract"`
	...
	KeyVersion                   string    `json:"-" gorm:"type:varchar(64);not null;index:idx_provider_file_readiness_lookup,priority:5;comment:Managed consensus key version signing the readiness assertion"`
	...
	ExpiresAt                    time.Time `json:"expires_at" gorm:"not null;index:idx_provider_file_readiness_lookup,priority:6;comment:Hard expiry of this short-lived readiness assertion"`
...
func (evidence ManagedProviderFileReadinessEvidence) Validate() error {
	if !validManagedProviderFileDigest(evidence.TargetFingerprint) || !validManagedProviderFileDigest(evidence.ScopeFingerprint) || !validManagedProviderFileDigest(evidence.CredentialFingerprint) ||
		!validManagedProviderFileDigest(evidence.MaintenancePolicyFingerprint) || !validManagedProviderFileDigest(evidence.UploadPolicyFingerprint) ||
		...
		evidence.DatabaseMatrixVersion != ManagedProviderFileDatabaseMatrixVersion || evidence.PolicyVersion != ManagedProviderFilePolicyVersion ||
		evidence.AttestedAt.IsZero() || evidence.ExpiresAt.IsZero() ||
...
func GetManagedProviderFileReadinessEvidence(ctx context.Context, targetFingerprint, scopeFingerprint, credentialFingerprint, maintenancePolicyFingerprint, keyVersion string, now time.Time) (*ManagedProviderFileReadinessEvidence, error) {
	if ctx == nil || !validManagedProviderFileDigest(targetFingerprint) || !validManagedProviderFileDigest(scopeFingerprint) ||
		!validManagedProviderFileDigest(credentialFingerprint) || !validManagedProviderFileDigest(maintenancePolicyFingerprint) ||
		strings.TrimSpace(keyVersion) == "" || strings.TrimSpace(keyVersion) != keyVersion || now.IsZero() {
		return nil, gorm.ErrRecordNotFound
	}
	var evidence ManagedProviderFileReadinessEvidence
	err := DB.WithContext(ctx).Where(
		"target_fingerprint = ? AND scope_fingerprint = ? AND credential_fingerprint = ? AND maintenance_policy_fingerprint = ? AND key_version = ? AND expires_at > ?",
		targetFingerprint, scopeFingerprint, credentialFingerprint, maintenancePolicyFingerprint, keyVersion, now.UTC(),

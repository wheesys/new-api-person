...
type Model struct {
	...
	VendorID     int            `json:"-" gorm:"index"`
	Endpoints    string         `json:"endpoints,omitempty" gorm:"type:text"`
	BasePrice    *float64       `json:"base_price,omitempty" gorm:"comment:Model fixed base price in USD per request"`
...
func (mi *Model) Update() error {
	...
	return DB.Model(&Model{}).Where("id = ?", mi.Id).
		Select("model_name", "description", "icon", "tags", "endpoints", "base_price", "status", "sync_official", "name_rule", "updated_time").
		Updates(mi).Error
}
